    ...
    return (
        <body>
            ...
            <div>
                ...
                Andi summerizes it's findings, giving simple answers to it's users. While Andi operates across interfaces, we are
                designed specifically for their mobile application. In line with it's product vision, Andi is primarily for users who
                are no longer interested in being the product - specifically people frustrated by endless ads, data tracking, and
                convoluted search results. This encapsulates a large segment of potential customers, but as a tech minded alterative to
                the current industry giants, Andi is likely most relevant to users who are digitally native, frequently use search engines
                like Google or Yahoo (and are familiar with their layout), and are interested in need and actively searching for a better
                alterative. Andi would change the game - with users no longer needing to deal with endless adverts polluting their
                experience, more direct & accurate answers based on searches (helpful for researchers/frequent searchers), and also a
                friendlier UI that encourages exploration and highlights the most crucial information.
            </div>
            <div className='bricks'>
                <div className='brick'>
                    <div className='subhead'>Part 1: Sketching</div>
                    <div className='disc'>
                    We brainstormed what Andi could look like in the following sketches, ranging from a persona that engages with the user to a
                    traditional search engine interface. We also played with the way resluts are shared and organized. While doing this, we
                    tried to keep in mind what layout would be easiest for a new user to apply their existing mental maps to, while also
                    communicating Andi's difference from it's competitors.
                    </div>
